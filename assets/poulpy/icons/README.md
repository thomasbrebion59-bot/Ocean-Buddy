# Petits Poulpy — activités et niveaux

Douze illustrations créées avec l’outil intégré **image_gen**, à partir de la planche `Designer-40.png` fournie par le propriétaire d’Ocean Buddy. Les prompts et les corrections sont conservés dans [prompts.json](prompts.json).

- Activités : surf, bodyboard, baignade, paddle, kayak, snorkeling, plongée, kitesurf et windsurf.
- Niveaux : débutant, intermédiaire et expert.
- Identité : Poulpy masculin, orange, heureux, accessoires propres à chaque situation.
- Export : JPEG 384 × 384, encodage et réduction avec `sips`. Le lot pèse environ 644 Ko. Les PNG d’origine restent dans les fichiers générés de la session.

Les illustrations sont accompagnées de libellés textuels : elles ne remplacent pas le nom de l’activité ou du niveau. Les petits visuels utilisent un texte alternatif vide quand le libellé voisin porte déjà le sens. Les paysages des spots restent des photographies réelles.
