...
(() => {
  ...
  const variants = {
    ...
    travel: {name:'L’explorateur', caption:'Une carte, des envies et le large devant toi.', label:'Poulpy en voyage'},
  ...
  const src = key => `assets/poulpy/${Object.hasOwn(variants,key)?key:'travel'}.webp`;
  const image = (key,cls='',lazy=true) => `<img class="${cls}" src="${src(key)}" alt="" width="640" height="640" decoding="async" ${lazy?'loading="lazy"':''}>`;
  ...
  function setSvg(selector,key) {
    ...
    if(current?.getAttribute('href')===src(key))return;
    target.innerHTML=`<image href="${src(key)}" x="0" y="0" width="140" height="140" preserveAspectRatio="xMidYMid meet"/>`;
    target.setAttribute('role','img');target.setAttribute('aria-label',variants[key].label);
  }
  function refresh() {
    const key=context();
    setSvg('#octoHi',sportVariant(activeSport));setSvg('#octo',sportVariant(activeSport));
  ...
  const activities=[
    ...
    {key:'travel',sport:null,title:'Pars un peu plus loin.',label:'Surf trips',copy:'Tes spots. Ton itinéraire.'}
  ];
  ...
  worldNote.innerHTML=`${image('travel')}<div><span class="section-kicker">LE CONSEIL DE POULPY</span><p>Choisis un horizon.<br><b>On trouve ton spot ensemble.</b></p></div><span class="world-note-step">01 <i>/</i> DESTINATION</span>`;
  ...
  window.OceanPoulpy={refresh,src,sportVariant,context,decorateOnboarding,focusOnboarding};
