...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.TripModel=api})(typeof window!=='undefined'?window:this,function(){
  ...
  function load(storage,knownIds){
    ...
    return data.trips.map(t=>{
      ...
      const base={...t,steps:t.steps.filter(s=>s&&knownIds.includes(s.spotId)).map(s=>({id:text(s.id),spotId:s.spotId,date:text(s.date,10),notes:text(s.notes,1000)})),checklist:t.checklist.filter(c=>c&&typeof c.id==='string').map(c=>({id:text(c.id),label:text(c.label,160),done:!!c.done})),archived:!!t.archived,deleted:!!t.deleted};
  ...
  return {KEY,uid,create,edit,addStep,editStep,moveStep,removeTrip,restoreTrip,load,save,duration,defaultChecklist};
});
