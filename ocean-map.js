...
(() => {
  ...
  function preview(s){
    ...
    const image=photo?.src?`<img class="map-preview-photo" src="${esc(photo.thumb||photo.src)}" alt="${esc(photo.caption||s.photoContext||s.name)}">`:'<div class="map-preview-photo map-preview-no-photo" role="img" aria-label="Photo du lieu à vérifier">Photo du lieu à vérifier</div>';
