...
(() => {
  ...
  function overview(saved){
    ...
    return `${saved.length?`<div class="trip-saved-grid">${saved.map(t=>`<button class="trip-saved" data-action="select" data-id="${esc(t.id)}"><img src="${esc(cover(t))}" alt="" loading="lazy"><span class="trip-saved-body"><small>${esc(t.destination||'Ton prochain horizon')}</small><b>${esc(t.name)}</b><span>${esc(range(t))}</span><em>${badge(t)} ${I.arrow}</em></span></button>`).join('')}</div>`:`<div class="trip-welcome"><img src="${esc(WORLD_PHOTOS.as.src)}" alt="Paysage des îles Raja Ampat"><div><span class="trip-pill">TON PROCHAIN GRAND SOUVENIR</span><h2>Une envie d’ailleurs ?<br>Fais-en un voyage.</h2><p>Des premiers spots repérés au sac prêt à partir,<br>tout ton projet au même endroit.</p><button class="trip-primary" data-action="new">Créer mon premier trip ${I.arrow}</button></div><span class="trip-stamp">SPOTS<br>→ SOUVENIRS</span></div>`}
  ...
  function tripDetail(t){
    ...
    return `<div class="trip-banner" style="--trip-cover:url('${esc(cover(t))}')"><div><span class="trip-pill">${esc(t.destination||'MON PROCHAIN VOYAGE')}</span><h2>${esc(t.name)}</h2><p>${I.calendar} ${esc(range(t))} <span>·</span> ${badge(t)}</p></div><button class="trip-glass" data-action="edit">Modifier le voyage</button></div>
      ...
      <div class="trip-tip"><img src="assets/design/poulpy.webp" alt=""><p><b>Le conseil de Poulpy</b>Garde de la souplesse dans ton itinéraire. Avant chaque session, ouvre la fiche du spot pour vérifier les conditions et les points de vigilance.</p></div></aside></div>`;
  }
  ...
  function openDialog(html){lastFocus=document.activeElement;modal.innerHTML=`<button class="trip-dialog-close" data-close aria-label="Fermer">${I.close}</button>${html}`;modal.showModal();modal.querySelector('input,select,button:not([data-close])')?.focus();}
  ...
  window.OceanTrips={count:()=>active().length,fromSpot,open:()=>{go('trips');render()}};
