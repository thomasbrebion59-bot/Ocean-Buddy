...
(() => {
  ...
  function tripDetail(t){
    ...
    return `<div class="trip-banner" style="--trip-cover:url('${esc(cover(t))}')"><div><span class="trip-pill">${esc(t.destination||'MON PROCHAIN VOYAGE')}</span><h2>${esc(t.name)}</h2><p>${I.calendar} ${esc(range(t))} <span>·</span> ${badge(t)}</p></div><button class="trip-glass" data-action="edit">Modifier le voyage</button></div>
      ...
      <div class="trip-tip"><img src="assets/poulpy/travel.webp" alt=""><p><b>Le conseil de Poulpy</b>Garde de la souplesse dans ton itinéraire. Avant chaque session, ouvre la fiche du spot pour vérifier les conditions et les points de vigilance.</p></div></aside></div>`;
  }
  ...
  function openDialog(html){lastFocus=document.activeElement;modal.innerHTML=`<img class="trip-dialog-poulpy" src="assets/poulpy/travel.webp" alt="" width="640" height="640"><button class="trip-dialog-close" data-close aria-label="Fermer">${I.close}</button>${html}`;modal.showModal();modal.querySelector('input,select,button:not([data-close])')?.focus();}
