...
(() => {
  ...
  window.OceanTrips={count:()=>active().length,fromSpot,open:()=>go('trips'),route:()=>({selected:current()?.id||null,view}),restoreRoute:r=>{selected=trips.some(t=>t.id===r?.selected&&!t.archived)?r.selected:null;view=r?.view==='map'?'map':'itinerary';}};
