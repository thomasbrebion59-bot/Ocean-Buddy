...
(() => {
  ...
  const routeDistance=t=>{let total=0,prev=null;t.steps.forEach(s=>{const c=COORDS[s.spotId];if(c&&prev)total+=distanceKm(prev,c);if(c)prev=c});return total||0;};
  ...
  function tripDetail(t){
    ...
    return `<div class="trip-banner" style="--trip-cover:url('${esc(cover(t))}')"><div><span class="trip-pill">${esc(t.destination||'MON PROCHAIN VOYAGE')}</span><h2>${esc(t.name)}</h2><p>${I.calendar} ${esc(range(t))} <span>·</span> ${badge(t)}</p></div><button class="trip-glass" data-action="edit">Modifier le voyage</button></div>
      <div class="trip-layout"><div class="trip-main"><div class="trip-toolbar"><div class="trip-view"><button data-action="view" data-view="itinerary" class="${view==='itinerary'?'active':''}">${I.route} Itinéraire</button><button data-action="view" data-view="map" class="${view==='map'?'active':''}">${I.map} Carte</button></div><span class="trip-route-distance">${t.steps.length>1?I.route+' '+prettyDistance(routeDistance(t))+' entre les spots':'Ajoute 2 spots pour mesurer le trajet'}</span><button class="trip-primary" data-action="add">${I.plus} Ajouter un spot</button></div>
  ...
  function drawMap(t){
    ...
    tripMap=L.map(host,{scrollWheelZoom:true,dragging:true,touchZoom:true,doubleClickZoom:true,boxZoom:true,keyboard:true,inertia:true});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>',maxZoom:18}).addTo(tripMap);
    const points=coords.map(({s,i,c})=>{const p=[c.lat,c.lon],prev=i>0?coords[i-1].c:null,leg=prev?distanceKm(prev,c):null;L.marker(p,{icon:L.divIcon({className:'trip-map-pin',html:String(i+1),iconSize:[32,32]})}).addTo(tripMap).bindPopup(`<b>${esc(spot(s.spotId).name)}</b><br>${s.date?esc(date(s.date)):'Date à définir'}${leg!==null?`<br><span>${prettyDistance(leg)} depuis l’étape ${i}</span>`:''}`);return p;});
    if(points.length>1){L.polyline(points,{color:'#2154dc',weight:4,opacity:.85,dashArray:'8 9'}).addTo(tripMap);points.slice(1).forEach((p,i)=>{const a=points[i],mid=[(a[0]+p[0])/2,(a[1]+p[1])/2],d=distanceKm(coords[i].c,coords[i+1].c);L.marker(mid,{interactive:false,icon:L.divIcon({className:'trip-leg-label-wrap',html:'<span class="trip-leg-label">'+prettyDistance(d)+'</span>',iconSize:[70,24],iconAnchor:[35,12]})}).addTo(tripMap)});tripMap.fitBounds(points,{padding:[55,55],maxZoom:11})}else tripMap.setView(points[0],11);
  }
