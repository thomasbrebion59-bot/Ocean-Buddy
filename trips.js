...
(() => {
  ...
  function overview(saved){
    ...
    return `${saved.length?`<div class="trip-saved-grid">${saved.map(t=>`<article class="trip-saved-card"><button class="trip-saved" data-action="select" data-id="${esc(t.id)}"><img src="${esc(cover(t))}" alt="" loading="lazy"><span class="trip-saved-body"><small>${esc(t.destination||'Ton prochain horizon')}</small><b>${esc(t.name)}</b><span>${esc(range(t))}</span><em>${badge(t)} ${I.arrow}</em></span></button><div class="trip-card-actions"><button class="trip-delete-action" data-action="delete" data-id="${esc(t.id)}" aria-label="Supprimer ${esc(t.name)}">${I.trash} Supprimer</button></div></article>`).join('')}</div>`:`<div class="trip-welcome"><img src="${esc(WORLD_PHOTOS.as.src)}" alt="Paysage des îles Raja Ampat" loading="lazy" decoding="async"><div><span class="trip-pill">LE CARNET DE TES ENVIES</span><h2>Ton prochain<br>grand départ.</h2><p>Repère tes spots, choisis tes étapes <br>et prépare ton sac.</p><button class="trip-primary" data-action="new">${trips.length?'Créer un nouveau trip':'Créer mon premier trip'} ${I.arrow}</button></div><figure class="trip-poulpy-postcard"><img src="assets/poulpy/scenes/travel-v2.webp" alt="Poulpy l’explorateur avec sa carte et sa boussole" width="960" height="640" loading="lazy" decoding="async"><figcaption>Prochaine escale : tes envies.</figcaption></figure><span class="trip-stamp">LE LARGE<br>NOUS ATTEND.</span></div>`}
  ...
  function tripDetail(t){
    ...
    const distance=routeDistance(t),hasArchived=t.steps.some(s=>!spot(s.spotId));
    return `<div class="trip-banner" style="--trip-cover:url('${esc(cover(t))}')"><div><span class="trip-pill">${esc(t.destination||'MON PROCHAIN VOYAGE')}</span><h2>${esc(t.name)}</h2><p>${I.calendar} ${esc(range(t))} <span>·</span> ${badge(t)}</p></div><button class="trip-glass" data-action="edit">Modifier le voyage</button></div>
      <div class="trip-layout"><div class="trip-main"><div class="trip-toolbar"><div class="trip-view"><button data-action="view" data-view="itinerary" class="${view==='itinerary'?'active':''}">${I.route} Itinéraire</button><button data-action="view" data-view="map" class="${view==='map'?'active':''}">${I.map} Carte</button></div><span class="trip-route-distance">${distance!==null?I.route+' '+prettyDistance(distance)+(hasArchived?' entre les étapes connues':' entre les spots'):'Ajoute 2 spots consécutifs pour mesurer le trajet'}</span><button class="trip-primary" data-action="add">${I.plus} Ajouter un spot</button></div>
      ${view==='map'?`<div id="tripMap" class="trip-map" aria-label="Carte des étapes"></div><p class="trip-map-note">La ligne relie les étapes consécutives dans l’ordre prévu. Elle ne représente pas un trajet routier.${hasArchived?' Les étapes archivées ne figurent pas sur la carte.':''}</p>`:`<div class="trip-steps">${t.steps.length?t.steps.map((s,i)=>stepCard(t,s,i)).join(''):`<div class="trip-no-steps">${I.route}<h3>Le premier spot donne le ton.</h3><p>Pioche dans les ${SPOTS.length} spots du catalogue pour commencer ton itinéraire.</p><button class="trip-primary" data-action="add">Choisir mon premier spot ${I.arrow}</button></div>`}</div>`}
      ...
      <div class="trip-tip"><img src="assets/poulpy/scenes/travel-v2.webp" alt="" loading="lazy" decoding="async"><p><b>Le conseil de Poulpy</b>Garde de la souplesse dans ton itinéraire. Avant chaque session, ouvre la fiche du spot pour vérifier les conditions et les points de vigilance.</p></div></aside></div>`;
  }
  function stepLogistics(s){
    const stepId=esc(s.id);
    return `<details class="trip-step-logistics" ${s.budget||s.accommodation||s.transport?'open':''}><summary>Budget, hébergement et transport de l’étape <small>privés</small></summary><div class="trip-step-logistics-fields"><label>Budget prévu (€)<input type="text" inputmode="decimal" maxlength="40" data-step-id="${stepId}" data-step-field="budget" data-focus="budget-${stepId}" value="${esc(s.budget||'')}" placeholder="0,00"></label><label>Hébergement<input type="text" maxlength="1000" data-step-id="${stepId}" data-step-field="accommodation" data-focus="accommodation-${stepId}" value="${esc(s.accommodation||'')}" placeholder="Adresse, réservation…"></label><label>Transport<input type="text" maxlength="1000" data-step-id="${stepId}" data-step-field="transport" data-focus="transport-${stepId}" value="${esc(s.transport||'')}" placeholder="Train, voiture, bateau…"></label></div></details>`;
  }
  function stepCard(t,s,i){
    const p=spot(s.spotId);
    if(!p){const name=s.legacy?.name||'Spot indisponible',loc=s.legacy?.loc||'Ancienne étape enregistrée';
      return `<article class="trip-step trip-step-archived"><div class="trip-step-number">${String(i+1).padStart(2,'0')}</div><div class="trip-step-photo" aria-hidden="true">${I.map}</div><div class="trip-step-info"><small>${esc(loc)} · Hors du catalogue actuel</small><h3>${esc(name)}</h3><span class="trip-level">Cette étape et tes notes sont conservées.</span><div class="trip-step-fields"><label>Date de l’étape<input type="date" aria-label="Date : ${esc(name)}" data-step-date="${esc(s.id)}" data-focus="date-${esc(s.id)}" value="${esc(s.date)}" ${t.start?`min="${t.start}"`:''} ${t.end?`max="${t.end}"`:''}></label><label>À prévoir<input type="text" aria-label="Notes : ${esc(name)}" maxlength="1000" data-step-notes="${esc(s.id)}" data-focus="step-${esc(s.id)}" value="${esc(s.notes)}"></label></div>${stepLogistics(s)}<button class="trip-primary" data-action="replace" data-id="${esc(s.id)}">Choisir un spot du catalogue pour cette étape</button></div><div class="trip-step-controls"><button data-action="up" data-id="${esc(s.id)}" ${i===0?'disabled':''} aria-label="Monter ${esc(name)}">↑</button><button data-action="down" data-id="${esc(s.id)}" ${i===t.steps.length-1?'disabled':''} aria-label="Descendre ${esc(name)}">↓</button><button data-action="remove" data-id="${esc(s.id)}" aria-label="Retirer ${esc(name)}">${I.close}</button></div></article>`;
    }
    const prev=i>0?spot(t.steps[i-1].spotId):null,leg=prev?distanceKm(COORDS[prev.id],COORDS[p.id]):null;
    return `<article class="trip-step"><div class="trip-step-number">${String(i+1).padStart(2,'0')}</div><div class="trip-step-photo"><img src="${esc(photo(p.id))}" alt="" loading="lazy"><button data-action="spot" data-spot="${p.id}">Voir le spot ↗</button></div><div class="trip-step-info"><small>${esc(p.loc)}${leg!==null?` · ${prettyDistance(leg)} depuis l’étape précédente`:''}</small><h3>${esc(p.name.split(' — ')[0])}</h3><span class="trip-level">${levelIcon(p.level)}${esc(lvlLabel[p.level])} · ${spotSports(p).slice(0,3).map(id=>esc(SPORTMAP[id].label)).join(' · ')}</span><div class="trip-step-fields"><label>Date de l’étape<input type="date" aria-label="Date : ${esc(p.name)}" data-step-date="${esc(s.id)}" data-focus="date-${esc(s.id)}" value="${esc(s.date)}" ${t.start?`min="${t.start}"`:''} ${t.end?`max="${t.end}"`:''}></label><label>À prévoir<input type="text" aria-label="Notes : ${esc(p.name)}" maxlength="1000" data-step-notes="${esc(s.id)}" data-focus="step-${esc(s.id)}" value="${esc(s.notes)}" placeholder="Session, accès, matériel…"></label></div>${stepLogistics(s)}</div><div class="trip-step-controls"><button data-action="up" data-id="${esc(s.id)}" ${i===0?'disabled':''} aria-label="Monter ${esc(p.name)}">↑</button><button data-action="down" data-id="${esc(s.id)}" ${i===t.steps.length-1?'disabled':''} aria-label="Descendre ${esc(p.name)}">↓</button><button data-action="remove" data-id="${esc(s.id)}" aria-label="Retirer ${esc(p.name)}">${I.close}</button></div></article>`;
  }
  const modal=document.createElement('dialog');modal.id='tripDialog';modal.className='trip-dialog';document.body.append(modal);
  function openDialog(html,initialFocus=null){lastFocus=document.activeElement;modal.innerHTML=`<img class="trip-dialog-poulpy" src="assets/poulpy/scenes/travel-v2.webp" alt="" width="640" height="640" loading="lazy" decoding="async"><button class="trip-dialog-close" data-close aria-label="Fermer">${I.close}</button>${html}`;const heading=modal.querySelector('h2');if(heading){heading.id='tripDialogTitle';modal.setAttribute('aria-labelledby',heading.id);}modal.showModal();modal.querySelector(initialFocus||'input,select,button:not([data-close])')?.focus();}
  ...
  function picker(replacingStepId=null){
    openDialog(`<span class="page-eyebrow">${replacingStepId?'REMPLACER UNE ANCIENNE ÉTAPE':'UNE NOUVELLE ÉTAPE'}</span><h2>${replacingStepId?'Quel spot du catalogue choisir ?':'Le prochain spot ?'}</h2>${replacingStepId?'<p>La date et les notes de cette étape seront conservées.</p>':''}<label class="sr-only" for="tripSpotSearch">Rechercher dans les spots</label><input id="tripSpotSearch" type="search" placeholder="Nom du spot, pays, région…" autocomplete="off"><div class="trip-picker-filters"><label for="tripRegion">Continent</label><select id="tripRegion"><option value="all">Tous les continents</option>${WORLDS.map(w=>`<option value="${w.id}">${w.lab}</option>`).join('')}</select><label for="tripCountry">Pays</label><select id="tripCountry" disabled><option value="all">Tous les pays</option></select><label class="trip-only-favs"><input id="tripOnlyFavs" type="checkbox"> Mes favoris</label></div><div id="tripSpotResults" class="trip-spot-results"></div>`);
  ...
  $('#tripContent').addEventListener('input',e=>{
    ...
    if(el.dataset.stepField){
      if(el.dataset.stepField==='budget'&&el.value&&!/^\d+(?:[.,]\d{1,2})?$/.test(el.value))return;
      update(t=>M.editStep(t,el.dataset.stepId,{[el.dataset.stepField]:el.value}),{draw:false});
    }
  });
  $('#tripContent').addEventListener('change',e=>{const el=e.target,t=current();if(!t)return;
    ...
    if(el.dataset.stepField){const s=t.steps.find(s=>s.id===el.dataset.stepId),key=el.dataset.stepField;if(!s||!update(t=>M.editStep(t,s.id,{[key]:el.value}),{draw:false}))el.value=s?.[key]||'';}
  });
