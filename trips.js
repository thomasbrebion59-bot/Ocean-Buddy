...
(() => {
  ...
  const I={plus:icon('M12 5v14M5 12h14'),route:icon('M5 5a2 2 0 1 1 0 4 2 2 0 0 1 0-4m14 10a2 2 0 1 1 0 4 2 2 0 0 1 0-4M5 9v4a4 4 0 0 0 4 4h4m-2-10h4a4 4 0 0 1 4 4v4'),calendar:icon('M5 5h14v16H5zM8 3v4m8-4v4M5 10h14'),bag:icon('M4 8h16v13H4zM8 8V3h8v5'),check:icon('m5 12 4 4 10-10'),arrow:icon('M5 12h14m-6-6 6 6-6 6'),close:icon('m6 6 12 12M6 18 18 6'),map:icon('m3 5 6-2 6 2 6-2v16l-6 2-6-2-6 2zm6-2v16m6-14v16')};
  const ideas=[
  ...
  const active=()=>trips.filter(t=>!t.archived), current=()=>trips.find(t=>t.id===selected&&!t.archived);
  ...
  function overview(saved){
    ...
    return `${saved.length?`<div class="trip-saved-grid">${saved.map(t=>`<button class="trip-saved" data-action="select" data-id="${esc(t.id)}"><img src="${esc(cover(t))}" alt="" loading="lazy"><span class="trip-saved-body"><small>${esc(t.destination||'Ton prochain horizon')}</small><b>${esc(t.name)}</b><span>${esc(range(t))}</span><em>${badge(t)} ${I.arrow}</em></span></button>`).join('')}</div>`:`<div class="trip-welcome"><img src="${esc(WORLD_PHOTOS.as.src)}" alt="Paysage des îles Raja Ampat"><div><span class="trip-pill">LE CARNET DE TES ENVIES</span><h2>Ton prochain<br>grand départ.</h2><p>Repère tes spots, choisis tes étapes<br>et prépare ton sac.</p><button class="trip-primary" data-action="new">Créer mon premier trip ${I.arrow}</button></div><figure class="trip-poulpy-postcard"><img src="assets/poulpy/scenes/travel.webp" alt="Poulpy l’explorateur avec sa carte et sa boussole" width="960" height="640"><figcaption>Prochaine escale : tes envies.</figcaption></figure><span class="trip-stamp">LE LARGE<br>NOUS ATTEND.</span></div>`}
      ...
      ${archived.length?`<details class="trip-archives"><summary>Voyages archivés (${archived.length})</summary>${archived.map(t=>`<div><span>${esc(t.name)}</span><button data-action="restore" data-id="${esc(t.id)}">Restaurer</button></div>`).join('')}</details>`:''}`;
  }
  ...
  $('#tripContent').addEventListener('click',e=>{
    ...
    if(a==='overview'){selected=null;render()}
    if(a==='select'){selected=id;view='itinerary';render();$('#screenWrap').scrollTop=0;}
    if(a==='add')picker();
    ...
    if(a==='archive'){const t=current();if(persist(trips.map(x=>x.id===t.id?{...x,archived:true}:x))){selected=null;render();toast('Voyage archivé. Tu peux le restaurer en bas de la page.');}}
    if(a==='restore'){if(persist(trips.map(t=>t.id===id?{...t,archived:false}:t))){selected=id;view='itinerary';render();$('#screenWrap').scrollTop=0;toast('Voyage restauré');}}
  ...
  window.OceanTrips={count:()=>active().length,fromSpot,open:()=>go('trips')};
