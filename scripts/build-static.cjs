...
for(const item of fs.readdirSync(root,{withFileTypes:true})){
  if(item.isFile()&&(/\.(html|css|ico)$/.test(item.name)||declaredScripts.has(item.name)||item.name==='.nojekyll'))fs.copyFileSync(path.join(root,item.name),path.join(dest,item.name));
}
