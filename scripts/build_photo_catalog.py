...
(ROOT/'photo-catalog.js').write_text('/* Real photographs. Sources and licences: photos.html. */\nwindow.SPOT_PHOTOS = '+json.dumps(catalog,ensure_ascii=False,separators=(',',':'))+';\nwindow.SPOT_GALLERIES = '+json.dumps({id:[{k:p[k] for k in ['src','source','author','license','licenseUrl']} for p in rows] for id,rows in galleries.items()},ensure_ascii=False,separators=(',',':'))+';\n')
...
spot_cards += [card(id+'-gallery-'+str(i),p,'Galerie',names.get(id,id)+' · autre regard') for id,rows in galleries.items() for i,p in enumerate(rows,2)]
