...
spots = json.loads((ROOT/'data/catalog.json').read_text())
...
fields = ['src','source','author','license','licenseUrl','caption','label','view','position','featured','thumb','width','height']
catalog = {k:{field:p[field] for field in fields if field in p} for k,p in meta.items()}
(ROOT/'photo-catalog.js').write_text('/* Real photographs. Sources and licences: photos.html. */\nwindow.SPOT_PHOTOS = '+json.dumps(catalog,ensure_ascii=False,separators=(',',':'))+';\nwindow.SPOT_GALLERIES = '+json.dumps({id:[{k:p[k] for k in fields if k in p} for p in rows] for id,rows in galleries.items()},ensure_ascii=False,separators=(',',':'))+';\n')
world = {k:{**catalog[id],'place':place} for k,(id,place) in worlds.items()}
if '--update-world' in sys.argv:
...
spot_cards += [card(id+'-gallery-'+str(i),p,'Galerie',names.get(id,id)+' · '+p.get('label','autre regard')) for id,rows in galleries.items() for i,p in enumerate(rows,2)]
