...
(() => {
  ...
  function update(s,act){
    ...
    $('#fieldAtAGlance').innerHTML=[['activity','Ton activité',SPORTMAP[act]?.label||'À choisir',sportIcon(act)],['water','Le cadre',isInland(s)?'Eau douce':'Mer & océan',icon('water')],['level','Repère de niveau',level,icon('level')]].map(([key,label,value,art])=>`<div class="field-fact-card fact-${key}"><span class="field-fact-icon">${art}</span><div><small>${label}</small><b>${esc(value)}</b></div></div>`).join('');
