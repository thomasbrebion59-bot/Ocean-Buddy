...
(() => {
  ...
  function render(){
    ...
    const feed=online?(state.feed.map(e=>card(e)).join('')||'<p class="community-empty">Le fil est encore calme.</p>'):`<div class="community-service-state" role="status"><b>${state.status==='checking'?'Connexion en cours':'Communauté indisponible'}</b><p>${esc(state.status==='setup'?'La communauté en ligne attend sa configuration. Tes voyages restent privés.':state.error||'Le service est momentanément indisponible.')}</p>${state.status==='checking'?'':'<button data-retry>Réessayer</button>'}</div>`;
  ...
  function mountSpot(id){
    ...
    if(state.status!=='ready'){box.innerHTML='<h3>Avis communautaires indisponibles</h3><p class="community-muted">Le service communautaire n’est pas ouvert pour le moment. Les avis publics ne peuvent pas être chargés.</p>';return}
